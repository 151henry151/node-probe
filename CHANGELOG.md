...
- **`README.md`**: document tracefs **`events/`** visibility (**`syscalls`**, **`sock`**) for non-root loaders and reference **`scripts/tracefs-perms.service`** installation steps.

## [0.1.7] - 2026-05-09

### Changed

- Drop **`priv/static/cache_manifest.json`** from git tracking — it is listed in **`.gitignore`** and generated by **`mix phx.digest`** / **`mix assets.deploy`** on each deploy.
